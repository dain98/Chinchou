...
class Osu:
    ...
    def __init__(self, bot):
        ...
        if 'cloudinary_auth_info' in self.api_keys.keys():
            cloudinary.config(
                cloud_name = self.api_keys['cloudinary_auth_info']['api_name'],
                api_key = self.api_keys['cloudinary_auth_info']['api_key'],
                api_secret = self.api_keys['cloudinary_auth_info']['api_secret'])
    ...
    @setkey.command(name="cloudinary", pass_context=True)
    @checks.is_owner()
    async def setimgur(self, ctx):
        await self.bot.whisper("Type your cloudinary api name. You can reply here.")
        name = await self.bot.wait_for_message(timeout=30, author=ctx.message.author)
        if name is None:
            return
        await self.bot.whisper("Type your cloudinary client key. You can reply here.")
        client_id = await self.bot.wait_for_message(timeout=30, author=ctx.message.author)
        if client_id is None:
            return
        await self.bot.whisper("Type your cloudinary client secret.")
        client_secret = await self.bot.wait_for_message(timeout=30, author=ctx.message.author)
        if client_secret is None:
            return
        self.api_keys['cloudinary_auth_info'] = {}
        self.api_keys['cloudinary_auth_info']['api_name'] = name
        self.api_keys['cloudinary_auth_info']['api_key'] = client_id
        self.api_keys['cloudinary_auth_info']['api_secret'] = client_secret
        fileIO("data/osu/apikey.json", "save", self.api_keys)
        await self.bot.whisper("Cloudinary API details added. :white_check_mark:")
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def sig(self, ctx, username, mode=0):
        user = ctx.message.author
        url = 'http://lemmmy.pw/osusig/sig.php?colour=hexee8833&uname={}&pp=2&darktriangles&onlineindicator=undefined&xpbar&xpbarhex&mode={}'.format(username, mode)
    ...
    async def osuleaderboard(self, ctx, *option):
        ...
        try:
            ...
            for member in members:
                ...
                if not player:
                    player = db.user_settings.find_one({'osu_user_id': osu_id})
                    try:
    ...
    async def screenshot(self, ctx):
        ...
        key_name = "screenshot"
        server = ctx.message.server
        option = await self._handle_option(key_name, server.id)
    ...
    async def user(self, ctx, *, username):
        ...
        if not await self._check_user_exists(user):
            try:
    ...
    async def _process_user_info(self, ctx, usernames, gamemode:int):
        ...
        # determine api to use
        usernames, api = await self._determine_api(server, usernames)
        ...
        count_valid = 0
        for i in range(len(final_usernames))[:5]:
            #try:
    ...
    async def _determine_api(self, server, inputs):
    ...
    async def _process_user_recent(self, ctx, inputs):
        ...
        username, api = await self._determine_api(server, list(inputs))
        username = username[0]
        ...
        if gamemode == -1:
            target_id = await self._get_discord_id(username, api)
            if target_id != -1:
                user_setting = db.user_settings.find_one({'user_id':target_id})
                gamemode = user_setting['default_gamemode']
            elif target_id == -1 and await self._check_user_exists(user):
    ...
    async def _get_discord_id(self, username:str, api:str):
    ...
    async def _process_map_score(self, ctx, map_id, inputs):
        ...
        # determine api to use
        _, api = await self._determine_api(server, list(inputs))
        ...
        username, _ = await self._determine_api(server, list(inputs))
        username = username[0]
        # gives the final input for osu username
        username = await self._process_username(ctx, username)
    ...
    async def _process_username(self, ctx, username, disp_error = True, check_api = True):
        ...
        if not username:
            if await self._check_user_exists(user):
    ...
    async def _check_user_exists(self, user):
        find_user = db.user_settings.find_one({"user_id":user.id})
        if not find_user:
            return False
        return True
    ...
    async def _get_user_info(self, api:str, server, server_user, user, gamemode: int):
        ...
        if api == self.osu_settings["type"]["default"]:
            try:
                url = 'https://osu.ppy.sh/users/{}/{}'.format(user['username'], modes2[gamemode])
                soup = await get_web(url, parser = "lxml")
                script = soup.find("script", {"id": "json-user"}, type='application/json')
                # print(script)
                web_data = json.loads(script.text)
                # print(web_data)
                time = web_data["lastvisit"] # 2018-03-29T08:26:44+00:00
                # print("LAST VISIT: {}".format(time))
                lastvisit = datetime.datetime.strptime(time.replace(" UTC", ""), '%Y-%m-%dT%H:%M:%S+00:00')
                timeago = time_ago(datetime.datetime.now(), lastvisit)
        ...
        url = 'https://osu.ppy.sh/users/{}/{}'.format(user['username'], modes2[gamemode])
        soup = await get_web(url, parser = "lxml")
        script = soup.find("script", {"id": "json-rankHistory"}, type='application/json')
        web_data = json.loads(script.text)
        colour = (server_user.colour.r/255, server_user.colour.g/255, server_user.colour.b/255)
        url = await plot_rank(web_data["data"], color = colour)
        em.set_image(url = url)
        return em
        #except:
            #return None
    ...
    async def _get_recent(self, ctx, api, user, userrecent, gamemode:int):
        ...
        # grab beatmap image
        map_image_url = 'https://b.ppy.sh/thumb/{}.jpg'.format(beatmap['beatmapset_id'])
    ...
    async def find_link(self, message):
        ...
        for att in message.attachments:
            if 'screenshot' in str(att).lower() or 'unknown' in str(att).lower():
    ...
    async def process_beatmap(self, all_urls, message, server_options = None):
        ...
        for url, mods in all_urls:
            ...
            if beatmap_info:
                #print(include_graph)
                #print(message, beatmap_info, url, mods, extra_info, include_graph)
    ...
    async def _get_screenshot_map(self, url, unique_id):
        ...
        filepath = 'data/osu/temp/ss_{}.png'.format(rand_id)
        ...
        for mode in modes2:
            for attr in ['scoresBest', 'scoresRecent']:
                # get from site
                try:
                    url = 'https://osu.ppy.sh/users/{}/{}'.format(player_name, mode)
                    soup = await get_web(url, parser = "lxml")
                    script = soup.find("script", {"id": "json-extras"}, type='application/json')
                    # print(script)
                    web_data = json.loads(script.text)
                    # print(web_data)
                    success = True
                ...
                for play in plays:
                    ...
                    if 'title' in play['beatmapset']:
                        ...
                        # print(compiled_name, similarity)
                        # print(similarity)
                        if compiled_name in map_name or similarity >= 0.85: # high threshhold
                            print(player_name, compiled_name, similarity)
                            # no pp if not top
                            if attr == "best":
                                pp = play["pp"]
                            else:
        ...
        for result in search_results:
            if 'https://osu.ppy.sh/s/' in result or 'http://osu.ppy.sh/s/' in result:
                url = result
                beatmapset_id = result.replace('https://osu.ppy.sh/s/','').replace('http://osu.ppy.sh/s/','')
            ...
            elif 'https://osu.ppy.sh/b/' in result or 'http://osu.ppy.sh/b/' in result or 'http://osu.gatari.pw/b/' in result:
                beatmap_id = result.replace('https://osu.ppy.sh/b/','').replace('http://osu.ppy.sh/b/','').replace('http://osu.gatari.pw/b/','')
                beatmap_info = await get_beatmap(key, self.osu_settings["type"]["default"], beatmap_id)
    ...
    async def disp_beatmap(self, message, beatmap, beatmap_url:str, mods='', extra_info = None, graph = False, username = None):
        ...
        colour, colour_text = self._determine_status_color(int(beatmap[0]['approved']))
        ...
        # safe protect
        oppai_info = await get_pyoppai(beatmap[0]['beatmap_id'], accs = accs, mods = mod_num, plot = graph, color = '#{}'.format('{:02X}'.format(colour).rjust(6, '0')))
        ...
        map_image_url = 'https://b.ppy.sh/thumb/{}.jpg'.format(beatmap[0]['beatmapset_id'])
        em.set_thumbnail(url=map_image_url)
        if oppai_info and 'graph_url' in oppai_info:
            em.set_image(url=oppai_info['graph_url'])
            # print(oppai_info['graph_url'])
...
class Tracking:
    def __init__(self, bot):
        ...
        self.osu_settings = fileIO("data/osu/osu_settings.json", "load")
        self.api_keys = fileIO("data/osu/apikey.json", "load")
        self.max_requests = 450 # per minute, for tracking only
    ...
    """
    @checks.is_owner()
    @osutrack.command(no_pm=True)
    async def createlistdb(self):
        return
        print("Creating server/player association...")
        database = {}
        for track_user in db.track.find({}):
            #server.id -> channel.id -> players
            if "servers" in track_user:
                servers = track_user["servers"]
                for server_id in servers.keys():
                    if server_id not in database:
                        database[server_id] = {}
                        database[server_id]["users"] = {}
                        database[server_id]["server_id"] = server_id

                    try:
                        database[server_id]["users"][track_user["osu_id"]] = track_user["servers"][server_id]
                    except:
                        pass

        # put into the database
        for server_id in database.keys():
            db.track_server_list.insert_one(database[server_id])
        print("Finished creating s/p association.")"""
        ...
        for track_user in db.track.find({}):
            for mode in modes:
                if "servers" in track_user and server.id in track_user["servers"]:
                    if "top" in track_user["servers"][server.id] and mode in track_user["servers"][server.id]["top"]:
                        if "channel_id" in track_user["servers"][server.id]["top"][mode]:
                            if track_user["servers"][server.id]["top"][mode]["channel_id"] == channel.id:
                                channel_users.append("`{}`".format(track_user["username"]))
                                break

            if "servers" in track_user and server.id in track_user["servers"] and "channel" in track_user["servers"][server.id]:
                if track_user["servers"][server.id]["channel"] == channel.id:
                    channel_users.append("`{}`".format(track_user["username"]))

        too_long = False
        total_people = len(channel_users)
        channel_users = sorted(channel_users)

        if total_people == 0:
            channel_users.append("None.")
        elif len(channel_users) > 60:
            channel_users = channel_users[:60]
            too_long = True
        msg_users = ", ".join(channel_users)
        if too_long:
            msg_users += "..."
        em.add_field(name = "__#{} ({})__".format(channel.name, total_people), value = msg_users)
        em.set_footer(text = "Options/other channel information will come later... Suggest: https://discord.gg/aNKde73")
        await self.bot.say(embed = em)
            ...
            if not userinfo or len(userinfo) == 0:
                ...
                if not track_user:
                    if track_user_username and not track_user:
                        ...
                        new_json["servers"][server.id]["top"] = {}
                        new_json["servers"][server.id]["recent"] = {}
                        new_json["servers"][server.id]["options"] = options
                        for mode_num in options['gamemodes']:
                            new_json["servers"][server.id]["top"][modes[mode_num]] = {}
                            new_json["servers"][server.id]["top"][modes[mode_num]]["channel_id"] = channel.id
                            new_json["servers"][server.id]["top"][modes[mode_num]]["plays"] = options["plays"]
                        for mode_num in options['recent_gamemodes']:
                            new_json["servers"][server.id]["recent"][modes[mode_num]] = {}
                            new_json["servers"][server.id]["recent"][modes[mode_num]]["channel_id"] = channel.id
                            new_json["servers"][server.id]["recent"][modes[mode_num]]["plays"] = options["plays"]
                        if options["map_rank_achievements"] > 0:
                            new_json["servers"][server.id]["map_rank_achievements"] = {}
                            new_json["servers"][server.id]["map_rank_achievements"]["channel_id"] = channel.id
                            new_json["servers"][server.id]["map_rank_achievements"]["rank"] = options["map_rank_achievements"]
                        if options["map_updates"]:
                            new_json["servers"][server.id]["map_updates"] = {}
                            new_json["servers"][server.id]["map_updates"]["channel_id"] = channel.id
                        if options["user_updates"]:
                            new_json["servers"][server.id]["user_updates"] = {}
                            new_json["servers"][server.id]["user_updates"]["channel_id"] = channel.id
                        ...
                        if "recent" not in new_json["userinfo"]:
                            new_json["userinfo"]["recent"] = {}
                        for mode in modes:
                            new_json["userinfo"]["recent"][mode] = []
                        ...
                        # for node databases
                        new_json["num_copied"] = 0
                        db.new_track.insert_one(new_json)
                    ...
                    if server.id not in track_user["servers"]:
                        track_user["servers"][server.id] = {}
                        track_num += 1

                    count_add += 1

                    ## update server options, if -o in username, then start from scratch, otherwise update
                    if reset_options or "map_updates" not in track_user["servers"][server.id]: # do only what was defined in options/replace old format
                        gm_json = {}
                        recent_json = {}
                        mra = {}
                        mu = {}
                        ...
                        gm_json = track_user["servers"][server.id]["top"]
                        recent_json = track_user["servers"][server.id]["recent"]
                        mra = track_user["servers"][server.id]["map_rank_achievements"]
                        mu = track_user["servers"][server.id]["map_updates"]
                        uu = track_user["servers"][server.id]["user_updates"]

                    for mode_num in options['gamemodes']:
                        gm_json[modes[mode_num]] = {}
                        gm_json[modes[mode_num]]["channel_id"] = channel.id
                        gm_json[modes[mode_num]]["plays"] = options["plays"]

                    for mode_num in options['recent_gamemodes']:
                        recent_json[modes[mode_num]] = {}
                        recent_json[modes[mode_num]]["channel_id"] = channel.id
                        recent_json[modes[mode_num]]["plays"] = options["plays"]

                    if options["map_rank_achievements"] > 0:
                        mra["channel_id"] = channel.id
                        mra["rank"] = options["map_rank_achievements"]
                    if options["map_updates"]:
                        mu["channel_id"] = channel.id
                    if options["user_updates"]:
                        uu["channel_id"] = channel.id

                    db.track.update_one({"osu_id":osu_id}, {'$set':{
                        "servers.{}.options".format(server.id): options,
                        "servers.{}.top".format(server.id): gm_json,
                        "servers.{}.recent".format(server.id): recent_json,
                        "servers.{}.map_rank_achievements".format(server.id): mra,
                        "servers.{}.user_updates".format(server.id): uu,
                        "servers.{}.map_updates".format(server.id): mu,
                        }})
                    msg += "**`{}` now tracking on `#{}`. {}**\n".format(username, channel.name, self._display_options(options))

                    # test
                    """
                    track_info = db.track.find_one({"osu_id":osu_id})
                    if "plays" in track_info:
                        del track_info["plays"]
                        del track_info["userinfo"]
                        await self.bot.say(str(track_info))"""
    ...
    async def _get_server_track_num(self, server):
    ...
    def _display_options(self, options):
        ...
        other_options = ""
        recent_gamemodes_str = []
        for mode in options["recent_gamemodes"]:
            recent_gamemodes_str.append(str(mode))
        if options["recent_gamemodes"]:
            other_options += ", r: `{}`".format('|'.join(recent_gamemodes_str))
            if options["recent_pass"]:
                other_options += "|`p`"
        if "map_rank_achievements" in options and options["map_rank_achievements"]>0:
            other_options += ", mra: `{}`".format(options["map_rank_achievements"])
        if "user_updates" in options and options["user_updates"]:
            other_options += ", uu: `{}`".format(str(options["user_updates"]))
        if "map_updates" in options and options["map_updates"]:
            other_options += ", mu: `{}`".format(str(options["map_updates"]))

        msg += "(m: `{}`, t: `{}`{})".format('|'.join(gamemodes_str), str(options['plays']), other_options)
        return msg
    ...
    async def _get_options(self, usernames:tuple):
        ...
        options = {
                ...
                "recent_gamemodes": [],
                "recent_pass": True,
                "map_rank_achievements": 0,
                "user_updates": False,
                "map_updates": False,
            }

        if '-uu' in usernames:
            options["user_updates"] = True
            marker_loc = usernames.index('-uu')
            usernames = list(usernames)
            del usernames[marker_loc]
            usernames = tuple(usernames)

        if '-mu' in usernames:
            options["map_updates"] = True
            marker_loc = usernames.index('-mu')
            usernames = list(usernames)
            del usernames[marker_loc]
            usernames = tuple(usernames)

        if '-mra' in usernames:
            marker_loc = usernames.index('-mra')
            if len(usernames) - 1 == marker_loc:
                await self.bot.say("**Please provide the # <= rank you want to be notified about!**")
                return (None, usernames)

            num = usernames[marker_loc + 1]
            if not num.isdigit():
                await self.bot.say("**Please use only whole numbers 1-50!**")
                return (None, usernames)

            if int(num) < 0 or int(num) > 50:
                await self.bot.say("**Please only use numbers 1-50!**")
                return (None, usernames)

            options["map_rank_achievements"] = int(num)
            usernames = list(usernames)
        ...
        if '-mr' in usernames:
            ...
            for mode in modes:
                try:
                    ...
                    if "p" in mode:
                        options['recent_pass'] = True
                except:
                    pass

            if not final_modes:
                await self.bot.say("**Please enter valid modes/pass! e.g. `0123p`**")
                return (None, usernames)
        ...
        if '-m' in usernames:
            ...
            if not modes.isdigit():
                await self.bot.say("**Please use only whole numbers!**")
                return (None, usernames)
    ...
    async def remove(self, ctx, *usernames:str):
        ...
        for username in usernames:
            ...
            if not user_find:
                user_find = db.track.find_one({"osu_id":username})

            if user_find and "servers" in user_find and server.id in user_find["servers"]:
                del user_find["servers"][server.id]
                db.track.update_one({"username":username}, {"$unset":{"servers":user_find["servers"]}})
                msg+="**No longer tracking `{}` in the server.**\n".format(username, channel.name)
                count_remove += 1
            else:
                msg+="**`{}` is not currently being tracked.**\n".format(username)
        """
        if "plays" in user_find:
            del user_find["plays"]
            del user_find["userinfo"]
            await self.bot.say(str(user_find))"""
        if len(msg) > 500:
    ...
    async def play_tracker(self):
        ...
        while self == self.bot.get_cog('Tracking'):
            ...
            print("Time Started. Projected Cycle Time: {} Rate: {}".format(str(self.cycle_time/3), str(self.max_requests)))

            current_time = datetime.datetime.now()
            loop = asyncio.get_event_loop()
            should_cache = False
            for player in db.track.find({}, no_cursor_timeout=True)[:int(total_tracking/3)]:
...
# Written by Jams
async def get_pyoppai(map_id:str, accs=[100], mods=0, misses=0, combo=None, completion=None, fc=None, plot = False, color = 'blue'):
    ...
    if plot:
        #try:
        pyoppai_json['graph_url'] = await plot_map_stars(file_path, mods, color)
...
async def plot_rank(rank_data, color = 'blue'):
    ...
    img_id = random.randint(0, 50)
    filepath = "rank_{}.png".format(img_id)
    fig.savefig(filepath, transparent=True)
    plt.close()
    upload = cloudinary.uploader.upload(filepath)
    url = upload['url']
...
def check_files():
    api_keys = {"osu_api_key" : "", 'cloudinary_auth_info' : "", "puush_api_key": ""}
